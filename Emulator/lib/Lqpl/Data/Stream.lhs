...
module Lqpl.Data.Stream(Stream(..),
                    StreamT(..),
                    module Lqpl.Data.InfList) where
...
instance IL Stream where
      ...
      tl = strtl
      takeI 0 str = []
      takeI n str = strhd str : takeI (n-1) (strtl str)
      makeInfinite  = foldr Stream (error "May only convert unending lists")

      iterI f a = Stream a (iterI f ((strhd . f) a))
      pushI = Stream
      sumList il = Stream (strhd il) (fmap (+ strhd il)
                                      (sumList (strtl il)))
      unzipI (Stream (a,b) tail) = (Stream a (fstI tail), Stream b (sndI tail))
        where fstI::Stream (a,b) -> Stream a
              fstI = fst . unzipI
              sndI::Stream (a,b) -> Stream b
              sndI = snd . unzipI
      zipI (Stream a taila) (Stream b tailb) = Stream (a,b) (zipI taila tailb)
      zipWithI f (Stream a taila) (Stream b tailb) = Stream (f a b) (zipWithI f taila tailb)
...
instance IL (StreamT IO) where
      ...
      tl  = strtlT
      takeI 0 _  = []
      takeI n str = hd str : takeI (n-1) (strtlT str)
      makeInfinite = foldr  (StreamT . return)
                            (error "May only convert unending lists to infinite lists")

      iterI f a = StreamT (return a) (iterI f ((hd . f) a))
      pushI = StreamT . return
      sumList il = StreamT (strhdT il) (fmap  (+ hd il)
                                        (sumList (strtlT il)))
      unzipI (StreamT a tail) = (StreamT (liftM fst a) (fstI tail),
                                 StreamT (liftM snd a) (sndI tail))
        where  fstI::StreamT IO (a,b) -> StreamT IO a
               fstI = fst . unzipI
               sndI::StreamT IO (a,b) -> StreamT IO b
               sndI = snd . unzipI
      zipI (StreamT a taila) (StreamT b tailb) = StreamT (liftM2 pr a b) (zipI taila tailb)
        where pr a b = (a,b)
