...
%if false
\begin{code}
{-# LANGUAGE FlexibleInstances,RankNTypes,AllowAmbiguousTypes #-}
module Lqpl.Data.Basis(Basis(..),
             OurBasis(..),
             showQv,
             offdiag) where
import Data.List as List (elemIndex,length)
...
class (Eq a, Ord a) => Basis a where
    basis            ::  [a]
    ei               ::  a->Int
    ei b             =   fromMaybe (error "Nothing constructor shouldn't happen") (elemIndex b basis)
    ind              ::  a -> a -> Int
    --ind b c          =   length (basis::Basis a1 => [a1]) * ei b + ei c
    toggle           ::  a -> a
    --toggle b         =   basis !! ((1 + ei b) `mod` (length (basis::Basis a1 =>[a1])))
    isFirst          ::  a -> Bool
    isFirst          =   (==  head basis)
    assocBasis       ::  [b]-> [(a,b)]
