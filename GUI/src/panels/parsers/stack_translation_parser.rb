# encoding: utf-8
# Parse the stack translation xml
class StackTranslationParser < AbstractListPatternParser
  KVPATTERN = Regexp.new('<kvpair><key><string>(?<key>.*?)</string></key>'\
                         '<value><int>(?<value>\d*)</int></value></kvpair>')
  ...
  def self.kv_pairs_to_map(kvps)
    values_to_list(kvps, KVPATTERN, {}) do |ret, mdkv|
