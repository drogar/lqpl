# encoding: utf-8
# Parse the data stores
class DataPatternParser < AbstractListPatternParser
  CONS_PATTERN = Regexp.new('(<string>(?<constructor_name>[\w\d_]*)</string>' \
                             '<StackAddresses>(?<addresses>(<int>(\d*)</int>)*)</StackAddresses>)')

  LIST_PATTERN = /^(<int>(?<address>\d*)<\/int>)/
  ...
  def parse_list
    ...
    DataPatternParser.values_to_list constructors_string, CONS_PATTERN do |rval, mdata|
      ...
      fail InvalidInput,
           "Constructor '#{cons_name}' duplicated in algebraic data" if rval.assoc(cons_name)
  ...
  def self.parse_address_list(addresses_string)
    DataPatternParser.values_to_list(addresses_string, LIST_PATTERN) do |rval, md|
      ...
      fail InvalidInput,
           "StackAddress '#{elem}' duplicated for single constructor" if rval.include? elem
      rval << elem
    end
  end
end
