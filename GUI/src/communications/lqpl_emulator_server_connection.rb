...
class LqplEmulatorServerConnection < Connection
  ...
  def send_load_from_file(depth_multiplier, fname)
    ...
    File.open(fname, 'r') do |f|
      ...
      send_and_read_data "load #{depth_multiplier} #{TranslateLineEnds.new qpl_file_data}"
    end
  end

  def get_qstack(tree_depth = 5, recursion_depth = 1)
    send_and_read_data("get qstack  #{recursion_depth} #{tree_depth}\n")
  end

  def get_stack_translation(tree_depth = 5, recursion_depth = 1)
    send_and_read_data("get memorymap #{recursion_depth}  #{tree_depth}\n")
  end

  def get_classical_stack(tree_depth = 5, recursion_depth = 1)
    send_and_read_data("get classicalstack #{recursion_depth}  #{tree_depth}\n")
  end

  def get_dump(tree_depth = 5, recursion_depth = 1)
    send_and_read_data("get dump #{recursion_depth}  #{tree_depth}\n")
  end

  def do_step(step_size = 1, recursion_depth = 1)
    send_and_read_data "step #{step_size} #{recursion_depth}\n"
  end

  def do_run(recursion_depth = 1)
    send_and_read_data "run #{recursion_depth}\n"
  end

  def do_trim
    send_and_read_data "trim\n"
  end

  def code_pointer(recursion_depth = 1)
    send_and_read_data "get codepointer #{recursion_depth}"
  end

  def loaded_code(recursion_depth =  1)
    send_and_read_data "get code #{recursion_depth}"
  end
