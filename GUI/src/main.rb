...
def log_the_error(exception, thread = nil)
  ...
  show_error_dialog('Server(s) not found',
                    'LQPL requires the compiler server and emulator server to be installed on your
                    path. Please download or compile these and add them to your path, e.g.,
                    in /usr/local/bin. See further details at
                    http://pll.cpsc.ucalgary.ca/lqpl') if rexcep.class == ServerProcessNotFound
