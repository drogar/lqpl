...
When(/^I click the button "([\w\s]*)" (\d+) times? on the frame "([\w\s]*)"$/) do
  |button_text, count, frame_title|
...
Then(/^the button "([\w\s]*)" on the frame "([\w\s]*)" should be (dis|en)abled$/) do
  |button_text, frame_title, dis_or_en|
  frame_ref = set_and_return_frame_fixture(frame_title)
  the_button = frame_ref.button(JButtonMatcher.with_text button_text)
