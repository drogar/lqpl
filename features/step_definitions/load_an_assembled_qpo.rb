...
Then(/^the frame "([\w\s]*)" should (be|not be) visible$/) do |frame_title, visible|
  frame_fixture_element = frame_fixture(frame_title)
  expect(sleep_until_visibility(5, frame_fixture_element, (visible == 'be'))).to be true
end

When(/^I click the button "([\w\s]*)" (\d+) times? on the frame "([\w\s]*)"$/) do |button_text, count, frame_title|
  frame_ref = frame_fixture(frame_title)
